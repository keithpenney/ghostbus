                            ...
                            for m in range(len(node._parent.memories)):
                                if node._parent.memories[m].domain == parent_domain:
                                    ...
                                    if node.memories[n].size > 0:
                                        #print(f"    +0405 Adding {node.memories[n].name} {node.memories[n].hierarchy} to memory {node._parent.label}")
                                        node._parent.memories[m].add_item(node.memories[n])
                                    else:
                                        #print(f"    +0405 SKIPPING {node.memories[n].name} {node.memories[n].hierarchy}")
                                        pass
                                    added = True
                            if not added:
            ...
            for n in range(len(node.memories)):
                if node.memories[n] is None:
                    ...
                    if node.memories[n].size == 0:
                        todelete.append(n)
            for n in todelete:
                #print(f"-0405 deleting {node.memories[n].label} {node.memories[n].hierarchy}")
                del node.memories[n]
        return top_memories
...
class GhostBusser():
    ...
    def GBRegister_Helper(self, module_name, net_dict):
        ...
        source      = net_dict.get('src', None)
        _range      = net_dict["range"]
        rangestr    = net_dict.get("range_str", None)
        initval     = net_dict['initval']
        depth       = net_dict.get("depth", None)
        array_str   = net_dict.get("depth_str", None)
        index_hi, index_lo = _range
        if index_hi is None:
            index_hi = 0
        dw = index_hi + 1
        ...
        if access is not None:
            ...
            reg.configFromTokens(strobe=strobe, alias=alias, domain=domain, addr=addr,
                                 initval=initval, signed=signed, genblock=generate)
            if genfor:
        ...
        if reftype == self._REFTYPE_CSR:
            bustop = self._handleBus(netname, dw, source, generate=generate, signed=signed, token_dict=token_dict, rangestr=rangestr)
        ...
        if write_strobe is not None:
            ...
            self.associated_strobes[netname] = (write_strobe, False)
        elif read_strobe is not None:
            self.associated_strobes[netname] = (read_strobe, True)
        return bustop
    ...
    def digestRegs(self, mod_dict, mod_hash, module_name=None):
        ...
        for net_dict in self.parser.gbnetsIterator(mod_dict):
            bustop = self.GBRegister_Helper(module_name, net_dict)
            if bustop is None:
                continue
        ...
        self.associateStrobes(self.associated_strobes)
        return
    ...
    def DEPRECATED_digestMems(self, mod_dict, mod_hash, module_name=None):
        # Check for RAMs
            ...
            for memname, mem_dict in memories.items():
                ...
                if gen_block is not None:
                    ...
                    if gen_index is None:
                        ...
                        #if generate is None:
                        #    raise GhostbusException(f"Failed to find for-loop for {gen_netname}")
                signed = mem_dict.get("signed", None)
                ...
                if access is not None:
                    ...
                    if gen_block is not None and gen_index is not None:
                        ...
                        if self.mrs.get(busname, None) is None:
                            ...
                            printd("2: created mr label {} ({})".format(self.mrs[busname].label, mod_hash))
    ...
    def assembleBusses(self, mod_hash):
        ...
        for ref in generates:
            if self.mrs.get(ref.domain, None) is None:
                self.mrs[ref.domain] = GBMemoryRegionStager(label=module_name, hierarchy=(module_name,), domain=ref.domain)
                printd("1: created mr label {} {} ({})".format(ref.domain, self.mrs[ref.domain].label, mod_hash))
            self.mrs[ref.domain].add(width=ref.aw, ref=ref, addr=ref.manual_addr)
        passengers = self._resolvePassengers()
        for passenger in passengers:
            added = False
            if self.mrs.get(passenger.domain, None) is None:
                self.mrs[passenger.domain] = GBMemoryRegionStager(label=module_name, hierarchy=(module_name,), domain=passenger.domain)
    ...
    def _resolveDrivers(self):
        ...
        for net_entry in self._bus_drivers:
            netname, dw, portnames, domain, source, association, generate, rangestr = net_entry
        ...
        for domain, entries in bus_dict.items():
            ...
            for entry in entries:
                ...
                #rangestr = getUnparsedWidthRange(source) # KILL. Moved.
    ...
    def _resolvePassengers(self):
        ...
        for net_entry in self._bus_passengers:
            netname, dw, portnames, instnames, domain, source, addr, association, generate, signed, rangestr = net_entry
    ...
    def _handleBus(self, netname, dw, source, generate=None, signed=None, token_dict={}, rangestr=(None,None)):
        ...
        if bustop:
            if domain is not None:
                ...
                if len(instnames) > 0:
                    domain = instnames[0]
            self._bus_drivers.append((netname, dw, portnames, domain, source, association, generate, rangestr))
            ...
            self._bus_passengers.append((netname, dw, portnames, instnames, domain, source, addr, association, generate, signed, rangestr))
        return bustop
        ...
        for block_name, block_info in self._generates.items():
            ...
            for reftype in ("csrs", "rams"):
                ...
                for netname, netdict in block_info[reftype].items():
                    ...
                    #aw = ref.aw
                    # Setting block_aw happens in _resolve_pass_generates
    ...
    def build_modtree(self):
        ...
        for mod_key in dd_keys:
            ...
            for inst_name in instance_keys:
                ...
                inst = self.modtree.get(inst_key, None)
                #print(f"      Instance key: {dict_key}; self.modtree[inst_key] = {inst}")
